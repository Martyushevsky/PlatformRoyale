...
public class AutoRifleBullet : MonoBehaviour, IDamageDealer
{
    ...
    private int predictionStepsPerFrame = 1;
    private float yGravity = -10f;
    ...
    {
        var effect = Instantiate(impactEffect, new Vector3(hit.point.x, hit.point.y, -1), Quaternion.LookRotation(hit.normal));
        Destroy(effect, 1f);
        ...
        if (rb != null)
        {
            rb.AddForceAtPosition(bulletVelocity * 100, hit.point);
        }
